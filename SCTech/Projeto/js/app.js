...
function realizarBaixaEstoque(ped) {
    ...
    for (const item of (ped.itens || [])) {
        ...
        if (mat) mat.estoque_atual = Math.max(0, (mat.estoque_atual || 0) - item.quantidade);
    }
...
function renderDashboard() {
    ...
    const COLS = [
        ...
        { key: 'amb', label: 'Ambiente' }, { key: 'valor', label: 'Valor (R$)' }, { key: 'status', label: 'Status' }
    ];
    ...
    tbody.innerHTML = rows || `<tr><td colspan="6" style="text-align:center;color:#999;padding:20px;">${vazio}</td></tr>`;
...
function atualizarTotalPedido() {
    ...
    const total      = totalMat + totalItens + mao;
    const el = id => document.getElementById(id);
...
function salvarPedido() {
    ...
    const valor            = total_material + total_acessorios + maoObra;
    ...
    const dadosPedido = {
        ...
        maoObra, total_material, total_acessorios, valor,
        ...
        const cards = pedidosCol.length === 0 ? `<div class="kanban-empty">Nenhum pedido nesta etapa</div>`
            : pedidosCol.map(p => {
                ...
                return `<div class="kanban-card">
                    ...
                    <div class="kanban-card-cliente">${p.clienteNome}</div>
    ...
    container.innerHTML = `
        ...
        <div class="os-section">
            <table class="os-table">
                ...
                <tr><td class="os-th">Ambientes</td><td><strong>${ped.amb||'—'}</strong></td><td class="os-th">Endereço</td><td>${cliente&&cliente.end?cliente.end:'—'}</td></tr>
            </table>
        </div>
        ${ambientesHTML}${itensHTML}
        <div class="os-section"><div class="os-section-title">Observações</div><div class="os-obs-box"></div></div>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (document.getElementById('tb-clientes')) {
        const rows = db.clientes.map(c => `<tr><td>${c.nome}</td><td>${c.tel||'—'}</td><td>${c.end||'—'}</td></tr>`).join('');
        document.getElementById('tb-clientes').innerHTML = rows || '<tr><td colspan="3" style="text-align:center;color:#999;padding:20px;">Nenhum cliente cadastrado.</td></tr>';
    }
